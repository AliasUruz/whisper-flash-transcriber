...
2.  **Clone the repository:** Execute o comando abaixo para baixar todos os arquivos do projeto em uma pasta chamada `whisper-flash-transcriber`.
    ```bash
    git clone https://github.com/AliasUruz/whisper-flash-transcriber.git
    ```
...
3.  **Navigate to the project directory:** Change your current working directory in the terminal to the folder you just cloned:
    ...
    cd whisper-flash-transcriber
    ```
    Seu prompt agora deve indicar que você está na pasta `whisper-flash-transcriber`.
...
1.  **Crie o ambiente virtual:** Dentro do diretório `whisper-flash-transcriber`, execute:
    ```bash
    python -m venv venv
    ```
...
2.  **Activate the virtual environment:** You need to activate the virtual environment in each terminal session you use for this project.
    ...
    *   **On Git Bash or other Unix-like shells (like in VS Code terminal):**
        ...
        ```
    Após a ativação, o prompt exibirá `(venv)` no início, por exemplo `(venv) C:\caminho\para\whisper-flash-transcriber>`.
...
1.  **Inicie o script principal:** Com o ambiente virtual **ativado** no diretório `whisper-flash-transcriber`, execute:
    ```bash
...
2.  **Abra um Issue no GitHub:** Para obter suporte, utilize [https://github.com/AliasUruz/whisper-flash-transcriber/issues](https://github.com/AliasUruz/whisper-flash-transcriber/issues). Informe sistema operacional, versão do Python, etapas para reproduzir e mensagens de erro.
