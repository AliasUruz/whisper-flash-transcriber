...
3. **Install the required dependencies:**
   ...
   ```
   The pinned dependencies rely exclusively on the CTranslate2 runtime (`faster-whisper` + `ctranslate2`), so there is no
   heavyweight PyTorch dependency in the default installation.

### Optional: GPU Acceleration

For significantly faster transcription you can leverage an NVIDIA GPU. The CTranslate2 wheels ship with both CPU- and GPU-enabled variants. After installing the base requirements, install the CUDA build that matches your driver by following the [official CTranslate2 instructions](https://opennmt.net/CTranslate2/installation.html). Once the GPU wheel is installed the application automatically detects and uses it.
