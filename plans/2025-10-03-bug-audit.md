...
### 3. Payload legado `new_record_to_memory` continua sendo persistido, gerando estados contraditórios
- **Arquivos**: `src/ui_manager.py:2424`, `src/core.py:1133`
- **Impacto**: A UI ainda envia `new_record_to_memory`, que passa a ser persistido como `record_to_memory` em `config.json`. Essa flag não é mais lida pelo `AudioHandler`, mas sobrevive ao merge e pode confundir agentes ou usuários que editam o JSON manualmente, além de sobrescrever migrações futuras.
- **Reprodução**: Abra a janela de configurações, altere o modo de armazenamento e salve. Observe que `config.json` mantém `record_to_memory` mesmo após várias alternâncias.
- **Correção sugerida**:
  1. Remover o envio de `new_record_to_memory` e tratar o modo apenas via `new_record_storage_mode`.
  2. Em `ConfigManager.apply_updates`, descartar a chave legada caso algum cliente externo ainda a envie.
- **Checklist**:
  - [ ] Remover o payload legado na UI.
  - [ ] Filtrar `record_to_memory` em `apply_updates`.
  - [ ] Atualizar documentação (AGENTS.md) sinalizando a remoção.
